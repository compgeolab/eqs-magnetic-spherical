...
coordinates, which are mathematically valid regardless of scale.
The limitation does not arise from the Cartesian formulation
itself, but from representing data distributed over the curved
Earth on a planar surface. For small areas, this approximation is
adequate. However, as the spatial extent increases, neglecting
Earth curvature introduces geometric inconsistencies in distances,
areas, and relative positioning. In this context, spherical
formulations provide a geometrically consistent framework for
regional to continental-scale modeling.
...
The use of equivalent sources for modeling potential fields evolved through
various formulations from the late 1960s to the early 1980s, transitioning from
Cartesian point masses and lines of dipoles to spherical distributions. For
instance, \citet{Mayhew1979} successfully applied an equivalent source
representation using arrays of dipoles and spherical prisms to model and invert
satellite magnetic anomaly data. Building upon these foundational works,
...
expensive at high degrees, and to Fourier transforms, which require planar
regridding of spherical data. Their approach relates potential field anomalies
directly to a distribution of equivalent point sources on a sphere through
least squares matrix inversion. This formulation is numerically efficient
because, once the source coefficients are solved, operations such as
continuation, spatial differentiation, and differential magnetic pole
reductions can be carried out by simply recomputing the anomaly using the
corresponding geometric point source function. Moreover, they demonstrated
that magnetic anomalies could be differentially reduced to the pole directly
in the spherical domain, avoiding the distortions associated with Cartesian
reregistration.
...
\begin{bmatrix}
    1 & 0 & 0\\
    0 & \cos(\Delta\Phi) & \sin(\Delta\Phi) \\
    0 & -\sin(\Delta\Phi) & \cos(\Delta\Phi) 
\end{bmatrix}
\begin{bmatrix}
    m_E \\
    m_N \\
