...
Faz o block reduce para estimar a camada profunda.
Tamanho do bloco e justificativa.
Referenciar o dado reduzido na figura 2.
CV com dado todo da camada profunda.
Referenciar resultados do CV na figura 3.
Resultados do CV.
Aplicada parâmetros para inverter o dado reduzido todo.
Prevê nos dados completos e calcula o residuo (figura 2).

Agora fazer a camada rasa.
Usa gradient boosting (tamanho da janela).
Como são muitos dados, CV nesse conjunto ficaria muito lento.
Como camada profunda isola os efeitos de longa distância, podemos usar um
recorte do dado para o CV da rasa.
Parâmetros do crop (coordenadas, numero de dados, etc).
Descreve o CV (tamanho do bloco, ranges, etc).
Resultados do CV (citar figura 3).
Usa parametros para fazer a inversão dos resíduos todos.
Calcula residuos totais da inversão (figura 2, histograma).
...
inversion was then performed on the full .
The core of the methodology lies in its
...
The estimated shallow and deep layers were used to predict data onto a regular
grid with a spatial resolution of $1$~km, constant height of 500 m, and
covering a larger area than the original data domain.
The grid dimensions were $1223 \times 1057$, totalling $3,878,133$ points.
As well as the total-field anomaly, we also used the estimated layers to
predict the three components of the anomalous magnetic field.
The final gridded predictions are shown in Figure~\ref{fig_grid_results}a-d,
along with the true grid generated by the synthetic sources
(Figure~\ref{fig_grid_results}e-h), and the difference
between the true and predicted grids (Figure~\ref{fig_grid_results}i-l).
The quality of the inversion is highlighted by the final residual maps
...
the underlying geophysical signals in the area where data were available.
By combining the effects of both source layers, the method accurately
reconstructs not only the total-field anomaly but also its eastward ($B_E$),
...
datasets. The entire workflow, encompassing the inversion of $507,395$
observation points and prediction on the $3,878,133$ points of the grid, was
completed in under $120$ minutes.
This was achieved on a personal computer with an
Intel\textsuperscript{\textregistered} Core\textsuperscript{TM} i5-9300H CPU
@ $2.40$~GHz (8 cores) and $16$~GB of RAM.
