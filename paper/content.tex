...
spacing of $0.3^{\circ}$ in both latitude and longitude directions,
corresponding to approximately 33~km in the study area. This block size
represents a compromise between data compression and preservation of
...
resulting in 25 parameter combinations. Spatial blocks of approximately
111~km (1$^{\circ}$) were used to preserve spatial dependence between samples
...
surface exhibits a relatively broad minimum around this configuration,
...
Figure~\ref{fig_decomposition}b. The predicted deep-layer field was then
interpolated back onto the full observation coordinates, and the residuals
between the original data and the deep-layer prediction were computed,
these residuals constitute the input for the shallow-layer inversion.

The shallow layer was estimated using an iterative gradient-boosting
algorithm applied to the residuals of the deep layer. A moving window with a
width of approximately 220~km (2$^{\circ}$) was adopted to control the spatial
influence of each boosting iteration and ensure smooth convergence.

Due to the large number of observations, performing cross-validation on the
entire residual dataset would be computationally prohibitive. However, since
...
area contains a representative range of anomaly amplitudes and source
geometries and preserves the spatial variability required for robust
cross-validation.

A block K-fold cross-validation was then applied to this subset using spatial
blocks of approximately 12~km. This configuration ensures that neighboring
...
the deep layer, the RMSE surface exhibits a more localized minimum,
highlighting the increased sensitivity of near-surface sources to parameter
variations and the need for stronger regularization.

The selected parameters were then applied to invert the full residual
dataset, producing the shallow-layer model shown in
Figure~\ref{fig_decomposition}d. The final residuals, obtained after combining
the deep and shallow predictions, are presented in
...
The estimated deep and shallow layers were used to predict the magnetic field
onto a regular grid with a spatial resolution of 1~km, constant height of
...
final residuals in Figure~\ref{fig_grid_results}i--l. The residual maps exhibit
low amplitudes and lack of coherent structures, demonstrating that the model
successfully captures both regional and local magnetic signals within the
data coverage area.

By combining two source layers, the method accurately reconstructs the
multi-scale structure of the magnetic field, representing a significant
improvement over traditional single-layer approaches
\citep{Uppal2025, Li2020}.

The complete workflow, including the inversion of 507,395 observations and
prediction over 3,878,133 grid points, was completed in under 120~minutes on a
...
proposed methodology for large-scale magnetic datasets.
...
Basin, Brazil. The survey covers latitudes from $-24.14^{\circ}$ to
...
The original raw data were sampled at 100~Hz and subsequently decimated to 1~Hz
by averaging consecutive groups of 100 samples along each flight line.
Numerical variables were averaged, while categorical and identifier fields
...
addition, tie lines were removed because they exhibited corrupted and
...
sources modeling. After this preprocessing step, the resulting dataset is
illustrated in Figure~\ref{fig_real_blockmean}a.

To investigate the sensitivity of the method to variations in sampling
density and spatial resolution, three spatial decimation strategies were
tested, including block-mean spacings of $0.06^{\circ}$ (approximately 6.6~km)
Figure~\ref{fig_real_blockmean}b,$0.0023^{\circ}$ (approximately 250~m) 
Figure~\ref{fig_real_blockmean}c,and the complete dataset without
decimation. The latter was used to generate a final grid with 250~m
resolution.
For all decimation levels, a consistent multiscale processing strategy was
adopted. Spatial blocks of $0.45^{\circ}$ (approximately 50~km) were used for cross-validation
and parameter tuning of the deep layer, ensuring independence at regional
scales. For the shallow layer, block sizes of $0.18^{\circ}$ (approximately 20~km) were
employed to preserve local spatial variability while avoiding spatial
overfitting. In addition, a moving window with a width of $1.5^{\circ}$ was used in the
gradient-boosting procedure to control the amount of data processed in each
iteration, ensuring efficient memory usage and computational stability.
This unified configuration allows a direct assessment of the robustness of
the proposed methodology with respect to data density, while maintaining
consistent regularization and spatial constraints across all experiments.
The observed and predicted grids obtained from the different decimation
strategies are shown in Figure~\ref{fig_real_blockmean}d--f.
The coarsest block-mean decimation ($0.06^{\circ}$) Figure~\ref{fig_real_blockmean}d resulted in the shortest
processing time (approximately 52~minutes); however, it significantly
attenuated short-wavelength magnetic anomalies, compromising the recovery of
near-surface sources and limiting its practical applicability. In contrast,
the finest block-mean decimation ($0.0023^{\circ}$) Figure~\ref{fig_real_blockmean}e, corresponding to the target
grid resolution, required approximately 10~hours and 48~minutes and produced
results that are nearly indistinguishable from those obtained using the full
post-processed dataset, which required approximately 13~hours and 49~minutes.
Despite the higher computational cost, the $0.0023^{\circ}$ block-mean
decimation provides a substantial reduction in processing time (about four
hours) relative to the full dataset, while preserving the dominant
short- and long-wavelength magnetic features. As the block size decreases and
approaches the target grid resolution, the predicted magnetic fields exhibit
progressive convergence, indicating robustness with respect to sampling
density and spatial resolution.
Based on this trade-off between computational efficiency and spectral
preservation, the block spacing of $0.0023^{\circ}$ was selected for the
subsequent analyses. This dataset was therefore adopted for the full
application of the dual-layer methodology. From this stage onward, the
processing workflow was identical to that adopted for the synthetic dataset.
The deep layer was estimated from a block-reduced version of the selected
dataset using a spacing of $0.1^{\circ}$, corresponding to approximately
11~km. Cross-validation blocks with characteristic dimensions of
approximately 50~km were adopted in order to isolate long-wavelength magnetic
components and preserve spatial independence.
The hyperparameters of the deep layer were selected through a block K-fold
cross-validation procedure applied to the block-reduced dataset. The tested
...
relative depth of approximately 100~km and a damping value of 100. The error
surface exhibits a relatively broad minimum, indicating limited sensitivity
to moderate hyperparameter variations.
Using the selected configuration, the deep-layer inversion was performed
using the entire block-reduced dataset. The predicted deep-layer field was
interpolated back onto the full observation coordinates, and the residuals
between the original data and the deep-layer prediction were computed. These
residuals constitute the input for the shallow-layer inversion.

The shallow layer was estimated using an iterative gradient-boosting
algorithm applied to the deep-layer residuals. The moving window with a width of $1.5^{\circ}$ was adopted in the
gradient-boosting procedure to control the amount of data processed in each
window, thereby limiting memory consumption and ensuring that the computations
remained compatible with the available RAM resources.
...
A block K-fold cross-validation was applied to this subset using spatial
blocks of approximately 20~km. The tested hyperparameters comprised damping
...
optimal shallow-layer configuration, highlighting the increased sensitivity
of near-surface sources to parameter variations and the need for appropriate
regularization.

The selected parameters were then applied to invert the full residual
dataset, producing the shallow-layer model. The final residuals, obtained
...
magnetic field onto a regular grid with a spatial resolution of 250~m and a
...
The magnetic field norm highlights and centralizes the main anomaly patterns,
providing an integrated view of source distribution. The $B_E$ component
...
exhibits well-centered anomalies, consistent with the distribution observed
in the total-field and norm maps. Together, these results confirm the ability
of the proposed methodology to recover physically consistent multicomponent
magnetic fields across multiple spatial scales.
