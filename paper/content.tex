...
This study presented and systematically validated a spherical dual-layer
gradient-boosted equivalent sources framework for large-scale magnetic data
processing and multiscale field reconstruction. The proposed methodology
integrates multi-layer equivalent sources modeling
\citep{Uppal2025,Li2020}, gradient-boosted residual inversion
\citep{Soller2021}, and rigorous spatial cross-validation into a unified and
computationally efficient workflow.

The synthetic experiments demonstrated that the dual-layer configuration
...
fidelity.
Cross-validation results obtained from the synthetic dataset showed broad and
well-defined minima for the deep layer, indicating stable solutions and low
sensitivity to moderate parameter variations. In contrast, the shallow layer
exhibited more localized minima, reflecting the increased sensitivity of
near-surface sources to depth and regularization. These results highlight the
importance of multiscale parameter tuning and justify the adoption of separate
optimization strategies for regional and local components.

The application to real airborne magnetic data from the Paraná Basin
confirmed the scalability and robustness of the proposed workflow under
realistic conditions, including heterogeneous sampling density, acquisition
artifacts, and large data volumes. The preprocessing strategy, combining
temporal decimation, spatial block-mean reduction, and systematic removal of
corrupted tie lines, proved essential for ensuring numerical stability and
minimizing inversion bias.
A detailed analysis of different spatial decimation strategies demonstrated
that coarse block-mean spacings, although computationally efficient, lead to
significant attenuation of short-wavelength anomalies and compromise the
recovery of near-surface sources. Conversely, block spacings compatible with
the final grid resolution preserved both regional and local features while
reducing processing time by several hours relative to the full dataset. This
trade-off analysis provides practical guidelines for large-scale magnetic
data processing and highlights the importance of resolution-aware
preprocessing.
An important outcome of this work is the reliable reconstruction of the
magnetic field vector and its amplitude from total-field anomaly data. The
simultaneous recovery of the total-field anomaly, magnetic norm, and vector
components yielded physically coherent and spatially consistent fields, in
agreement with the theoretical framework proposed by \citet{Lima2009}. These
results confirm the validity of the implemented approach for vector field
reconstruction using physically constrained equivalent sources.

The availability of vector components provides substantial advantages over
traditional derivative-based interpretation techniques. Magnetic derivatives
are inherently sensitive to noise, interpolation artifacts, and uneven data
coverage, particularly in continental-scale surveys. In contrast, the
equivalent-source-based reconstruction produces smooth, physically
constrained fields that retain geological meaning while significantly
reducing noise amplification.

This property is particularly relevant for advanced interpretation and
inversion workflows. As demonstrated by Hidalgo-Gato, reliable vector
components and multiscale field representations provide stronger constraints
on source geometry, magnetization direction, and depth distribution than
scalar data alone. The proposed framework therefore offers a solid foundation
for subsequent structural mapping, susceptibility inversion, and integrated
geophysical modeling.

The combined analysis of synthetic and real datasets demonstrates that the
dual-layer strategy consistently outperforms traditional single-layer
approaches, in agreement with previous findings by \citet{Uppal2025} and
\citet{Li2020}. By explicitly accounting for multiscale source distributions,
the method reduces spectral leakage between regional and local components and
improves overall model interpretability.
...
and physically consistent solution for modern magnetic data processing. The
framework is capable of handling very large datasets while preserving
essential geological information and maintaining manageable computational
requirements.

Beyond its immediate application to aeromagnetic data, the proposed approach
provides a general platform for integrated geophysical analysis. Future
developments may include joint inversion with gravity and electromagnetic
data, adaptive window strategies, and machine-learning-assisted parameter
selection, further enhancing its applicability to complex geological
environments and continental-scale studies.
