...
\begin{figure}[!ht]
    \centering
    \includegraphics[width=1\linewidth]{figures/v_real_00023.png}
...
The original raw data were sampled at 100~Hz and subsequently decimated to 1~Hz by averaging consecutive groups of
100 samples along each flight line. Numerical variables were averaged, while
categorical and identifier fields were retained from the central record of
each block. Records containing corrupted or inconsistent values were
discarded. This procedure preserves along-line continuity while
significantly reducing the dataset size, in addition, tie lines were removed because they exhibited corrupted and
...
$0.0023^{\circ}$ (approximately 250~m), and the complete dataset without
...
overfitting. In addition, a moving window with a width of $1.5^{\circ}$ was
used in the gradient-boosting procedure to control the spatial influence of
each iteration and ensure stable convergence.
...
strategies are shown in Figure~\ref{fig_real_blockmean}. As the block size
decreases and approaches the target grid resolution, the predicted magnetic
fields exhibit progressive convergence, indicating robustness with respect
to sampling density and spatial resolution.

Based on this analysis, the block spacing of $0.0023^{\circ}$ was selected as a
compromise between computational efficiency and spatial resolution. This
dataset was subsequently used for the full application of the dual-layer
methodology. From this stage onward, the processing workflow was identical to
that adopted for the synthetic dataset.
...
The shallow layer was estimated using an iterative gradient-boosting
algorithm applied to the deep-layer residuals. A moving window with a width of
$1.5^{\circ}$ was adopted to control the spatial influence of each boosting
iteration and ensure smooth convergence.
...
The estimated deep and shallow layers were used to predict the magnetic field
onto a regular grid with a spatial resolution of 250~m and constant height of
500~m.

In addition to the total-field anomaly, the eastward ($B_E$),
northward ($B_N$), and upward ($B_U$) components were also predicted Figure~\ref{fig_real_components}.
