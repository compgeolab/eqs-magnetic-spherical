...
\begin{figure}[tb]
    ...
    \caption{Illustration of the dual-layer decomposition process.
        ...
        regional field.
        \textbf{c)} Shallow total-field anomaly, capturing the short-wavelength
        local anomalies.
...
\begin{figure}[!ht]
    ...
    \caption{Gridded interpolation results for the total-field anomaly and its
        ...
        fields, demonstrating the accuracy of the inversion.}
...
with different shapes, sizes, depths, and magnetizations.
The exact geometries and magnetizations of the sources can be found in the
supplementary material \citep{archive}.
A complex regional
field was modeled using a deep set of sources, to which zero-mean
Gaussian noise with XXXXXXXXXXXXXXXX nT standard deviation (XXXXX\% of the
maximum absolute value of the data) was added to simulate instrument and
environmental noise.
To replicate a realistic survey geometry, the magnetic field was sampled along
the flight lines of a real public domain aeromagnetic survey from the Paraná
Basin, Brazil. The original dataset, provided by Agência Nacional do Petróleo
(ANP) and available through the CPRM/REATE
portal (\url{https://reate.cprm.gov.br/anp/TERRESTRE}),
was preprocessed using a block-reduction operation, resulting in
$507,395$ data points whose coordinates were used for the synthetic data.
A regional field with inclination -25° and declination -20°, which is close to
the mean IGRF field in the survey area, was used to calculate the total-field
anomalies.
The resulting synthetic total-field anomaly data is shown in
Figure~\ref{fig_decomposition}a.

DESCREVER SETUP DA INVERSÃO. Produção dos dados para a camada profunda com um
blockreduce. Justificar o tamanho do bloco para a camada profunda.
Camada rasa onde ficam as fontes e tamanho da janela no gradient-boosting.
Descrever brevemente como foi o processo todo: block reduce, ajusta profunda,
faz residuos, adjusta rasa.

SEPARAR OS PROXIMOS DOIS PARAGRAFOS EM PROFUNDA E RASA AO INVÉS DE CV
E INVERSÃO.

Before performing the dual-layer inversion, a block K-fold cross-validation
strategy was employed to determine the optimal hyperparameters for both
layers of the model.
FALTOU AQUI: tamanho do bloco e justificar, ranges de parâmetros, crop para
o CV da rasa, tudo isso para rasa e profunda também.
For the deep layer (Figure~\ref{fig_crossvalidation}a), the lowest RMSE is
obtained for a relative depth of approximately $37$~km and a damping value of
about $10^{0}$.
ESSA PARTE É AO CONTRARIO (CHECAR NA FIGURA): This region of minimum error is relatively broad, indicating
a stable solution with limited sensitivity to small variations in the
hyperparameters.
The optimal depth is consistent with the expected scale of
the regional magnetic field and confirms that the deep layer effectively
captures the long-wavelength components of the anomaly.
For the shallow layer (Figure~\ref{fig_crossvalidation}b), the minimum RMSE is
reached at a relative depth of approximately $9$~km and a damping value of
about $10^{6}$. In this case, the RMSE minimum is more localized in the
hyperparameter space, reflecting the increased sensitivity of the shallow
layer to parameter variations. This behavior is expected for near-surface
sources, which generate short-wavelength anomalies and require stronger
regularization to prevent overfitting.

Using the hyperparameters selected by cross-validation, the dual-layer
inversion was then performed on the full . The core of the methodology lies in its
dual-layer approach, which is illustrated in
Figure~\ref{fig_decomposition}. This process separates the observed
total-field anomaly (Figure~\ref{fig_decomposition}a) into two distinct
components. First, a deep layer is fitted to a block-averaged version of the
data to isolate long-wavelength signals. This strategy effectively models
the smooth regional field (Figure~\ref{fig_decomposition}b) while reducing
the required number of sources without sacrificing quality
\citep{Uppal2025}. Subsequently, the iterative gradient-boosting algorithm is
used to fit a shallower layer of sources to the remaining residual signal.
This second layer models the short-wavelength anomalies associated with
local sources (Figure~\ref{fig_decomposition}c). The effectiveness of this
decomposition is evident, as the final residuals
(Figure~\ref{fig_decomposition}d) are significantly smaller than those from
the deep layer alone (Figure~\ref{fig_decomposition}e).

The predicted fields were interpolated onto a regular grid with a spatial
resolution of $1$~km. The resulting grid is defined in latitude, longitude,
and height, with a shape of $(3, 1223, 1057)$, corresponding to the height
dimension and the horizontal latitude--longitude grid. This grid contains a
total of $3,878,133$ points.

The final gridded predictions, shown in Figure~\ref{fig_grid_results},
confirm the accuracy of the dual-layer model. By combining the effects of
both source layers, the method accurately reconstructs not only the
total-field anomaly but also its eastward (Be), northward (Bn), and upward
(Bu) components. There is a correspondence between the predicted fields
(Figure~\ref{fig_grid_results}a-d) and the true synthetic fields
(Figure~\ref{fig_grid_results}e-h). The quality of the inversion is further
highlighted by the final residual maps
(Figure~\ref{fig_grid_results}i-l), where the low magnitude and lack of
coherent structures demonstrate that the model has successfully captured
the underlying geophysical signals. This represents a significant
improvement over single-layer approaches, which often struggle with
multi-scale sources and suffer from artifacts at the survey borders
\citep{Uppal2025}.

The low residuals observed in the final results
(Figure~\ref{fig_grid_results}i-l) provide a good fit and strong evidence of
the method's excellent predictive capabilities. Overall, the tests with
synthetic data show that the proposed dual-layer approach is efficient for
predicting both the components of the field and the anomalous magnetic field
based solely on total-field anomaly data \citep{Uppal2025}.

The computational efficiency of the method is significantly enhanced by the
use of the gradient-boosting algorithm, making it highly applicable to large
datasets. As described by \citet{Soler2021}, this technique prevents the need
to solve a single, massive linear system by iteratively processing the data
in smaller, overlapping windows. To demonstrate its performance, the entire
workflow, encompassing the inversion and forward modeling of $507{,}395$
observation points, was completed in under $120$ minutes. This was achieved
on a personal computer with an Intel\textsuperscript{\textregistered}
Core\textsuperscript{TM} i5-9300H CPU @ $2.40$~GHz (8 cores) and $16$~GB of RAM.
Such performance confirms the method's feasibility for large-scale surveys,
overcoming a significant computational hurdle that has traditionally
limited the equivalent source technique.
...
\begin{figure}[tb]
    ...
    (\textbf{a}) Total-field anomaly. (\textbf{b--d}) East, north, and upward
...
To evaluate the performance of the proposed spherical dual-layer
gradient-boosted equivalent sources method under realistic conditions,
we applied it to airborne magnetic data from the Paraná Basin, Brazil.
This dataset was selected due to its large spatial coverage and complex
geological setting, which provide a suitable testbed for regional-scale
magnetic modeling.

Before the modeling procedure, the dataset was preprocessed to improve
its consistency and numerical stability. Block-mean decimation was
applied to reduce the dataset size and mitigate the effects of supersampling.
In addition, tie lines were removed from the dataset
due to amplitude inconsistencies that introduce spurious variations in
the magnetic signal and systematic distortions, which could negatively
affect the equivalent sources modeling.

To investigate the sensitivity of the method to variations in sampling
density, different block-mean sizes were applied to generate multiple
gridded datasets. Figure~\ref{fig_real_blockmean} illustrates the effect
of varying block-mean sizes on the observed and predicted total-field
anomalies. Panels \textbf{a--d} show the observed total-field anomaly
gridded using progressively finer block sizes, while panels
\textbf{e--h} present the corresponding predicted anomalies obtained
using the spherical dual-layer gradient-boosted equivalent sources.

The progressive convergence observed in the predicted grids as the block
size decreases indicates that the proposed method is robust to changes
in data resolution and sampling density. Even for coarser decimations,
the main magnetic features are consistently recovered, demonstrating
the stability of the inversion procedure.

After defining the optimal decimation parameters, the method was applied
to a dense set of equivalent sources distributed at two depth levels.
This configuration allows the simultaneous representation of shallow
and deep magnetic sources, improving the flexibility of the model in
capturing multiscale geological structures.

The quality of the final magnetic field prediction was assessed by
analyzing the total-field anomaly and its vector components. Figure~
\ref{fig_real_components} shows the predicted total-field anomaly and
the three magnetic field components.

The predicted total-field anomaly reproduces the main regional magnetic
trends observed in the original data, while preserving important
short-wavelength features. The consistency between the vector
components further indicates that the spherical dual-layer
gradient-boosted equivalent sources method provides a physically
coherent representation of the magnetic field.

Overall, the real data application demonstrates that the proposed
approach is robust to sampling-density variations, capable of handling
large-scale datasets, and effective in modeling multiscale magnetic
anomalies. These results confirm its suitability for regional and
continental-scale magnetic data integration and processing.
...
recent advances in windowed inversion \citep{Soler2021} and dual layer equivalent
sources decomposition \citep{Uppal2025} to a spherical method, the proposed methodology
