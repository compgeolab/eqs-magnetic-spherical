...
with different shapes, sizes, depths, and magnetizations. A complex regional
field was modeled using a deep pattern of sources, to which a zero-mean
Gaussian noise was added to simulate instrument and environmental noise. To
replicate a realistic survey geometry, the magnetic field was sampled along
the flight lines of an aeromagnetic survey from the Paraná Basin. The
original dataset, provided by ANP and available through the CPRM/REATE
portal, was preprocessed using a block-reduce operation, resulting in
$507,395$ points whose coordinates were used for the simulation.
...
total of $3,878,133$ points and was used for visualization and
quantitative assessment of the model performance.
...
layers of the model. For the deep layer (Figure~\ref{fig_crossvalidation}a), the lowest RMSE is
obtained for a relative depth of approximately $37$~km and a damping value of
about $10^{0}$. This region of minimum error is relatively broad, indicating
a stable solution with limited sensitivity to small variations in the
hyperparameters. The optimal depth is consistent with the expected scale of
...
The contrast between the optimal depths obtained for the deep and shallow
layers highlights the multi-scale nature of the magnetic sources in the study
area. The deeper sources primarily control the long-wavelength regional
field, whereas the shallower sources account for localized anomalies. The
consistent location of the RMSE minima in the hyperparameter space further
demonstrates the robustness of the cross-validation procedure and supports
the reliability of the selected model configuration.

Using the hyperparameters selected by cross-validation, the dual-layer
inversion was then performed. The core of the methodology lies in its
...
applied to reduce data redundancy and mitigate the effects of uneven
sampling density. In addition, tie lines were removed from the dataset
...
the three magnetic field components in the local reference frame.
...
components of the magnetic field in the local reference frame. The
