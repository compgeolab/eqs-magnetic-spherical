...
reduction to the pole \citep{Gusp2009, Li2001, Silva1986}.
...
number of data.
Since modern airborne surveys generate large volumes of data, the solution of
these linear systems becomes demanding in terms of both processing time and
computer memory \citep{OliveiraJunior2023}.
To address this challenge, a variety of strategies have been developed to
improve computational efficiency.
These include methods based on moving data windows
\citep{Leao1989,Soler2021,Uppal2025}, sparsity-inducing transformations using
wavelets \citep{Li2010} INCLUIR MAIS ESTUDOS.

FALTA DIZER ALGO SOBRE CARTESIANO.
For geophysical studies spanning regional to continental scales, this reliance
on a Cartesian framework introduces a second, more fundamental limitation:
...
\citet{VonFrese1981} as a computationally efficient alternative to spherical
harmonics, which become computationally expensive at high degrees, and to
Fourier transforms, which require planar regridding of spherical data.
FALTOU FALAR MAIS SOBRE EFICIENCIA E COMO ELE FAZ.
Their formulation demonstrated that potential field anomalies could be
represented by equivalent point sources directly on the sphere, enabling
continuation, differentiation, and reduction-to-the-pole operations without
distorting the data. CHECAR SE ELE FEZ RTP MESMO.
More recently, \citet{Kother2015} extended this methods to global lithospheric
magnetic field modelling using satellite data in combination with spherical
harmonics.
By distributing equivalent monopoles on an icosahedral grid and applying robust
regularization techniques, they achieved localized and scalable representations
of the lithospheric field while preserving the ability to transform results
into for global comparisons.
DESCREVER MAIS SOBRE O MÉTODO: NUMERO DE FONTES, COMO FUNCIONA, OUTROS
TRABALHOS RELACIONADOS. MÉTODO ORIGINAL:  O'Brien M.S. Parker R.L. Regularized
geomagnetic field modelling using monopoles
...
COLOCAR UM BLA BLA BLA DO QUE MAIS FOI FEITO.
FALAR ALGO SOBRE VALIDACAO.
...
$\vec{\mathbf{B}}$ from a magnetic dipole source, based on the seminal works of
\citet{VonFrese1981} and \citet{Langel1998}.
The formulation requires the definition of three distinct coordinate systems
(Figure~\ref{fig_coordinate_systems}).
The primary reference is a geocentric Cartesian system $(X, Y, Z)$,
with origin at the center of the Earth, $Z$-axis coinciding with the Earth's
axis of rotation, $X$-axis along the equatorial plane and aligned with the
Greenwich meridian, and the $Y$-axis completing a right-handed system.
The geocentric coordinates can also be described in terms of spherical
coordinates $(r, \theta, \lambda)$, with $r$ being the radial distance, $\theta$
the colatitude, and $\lambda$ the longitude
(Figure~\ref{fig_coordinate_systems}a).
DESCREVER SISTEMA TOPOCENTRICO DOS PONTOS P E Q.
This geocentric spherical system is used to formulate the physics.
However, data are typically provided in a geodetic system $(h, \varphi,
\lambda)$ (Figure~\ref{fig_coordinate_systems}b), with $h$ being the height
above a reference ellipsoid (e.g., WGS84), $\varphi$ the geodetic latitude, and
$\lambda$ the longitude.
Figure~\ref{fig_coordinate_systems}b illustrates the geometric relationship
between the geodetic and geocentric latitudes, whose difference arises from the
Earth's oblateness.
Therefore, the practical application of the equivalent source model, formulated
in geocentric spherical coordinates, to real-world data, provided in a geodetic
system, necessitates transformations between these coordinate systems.
