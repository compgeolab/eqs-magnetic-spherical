...
a windowed inversion framework adapted for the Earth's spherical geometry.
...
approximately $1$ km, establishing the reference spatial resolution
against which all block-mean decimations are evaluated. Introducing this
scale at the outset is essential, since the grid ultimately defines the
smallest wavelength that the inversion can express, and therefore constrains
how much spatial detail the input dataset is able to contribute.
...
Measurements were originally acquired at approximately $100$ Hz,
producing extremely dense sampling along flight lines. To obtain a more
uniform and manageable dataset, we applied a $100{:}1$ temporal decimation by
averaging every 100 consecutive samples while preserving flight-line
continuity. Numerical variables were averaged, whereas categorical fields
(e.g., line identifiers) were retained from the central record of each block.
Records containing corrupted or inconsistent values were discarded. This
procedure effectively reduced the dataset to a $1$ Hz survey while
preserving the essential spatial characteristics of the acquisition. The
resulting dataset served as the input for the block-mean sensitivity tests
presented below.

In this first real-data experiment, we investigated how the block-mean size
influences the magnetic anomaly predicted by the dual-layer spherical
equivalent sources. To explore this behavior, we produced several decimated
versions of the dataset by varying the block-mean size from values well below
the final grid spacing to values comparable to it. Each dataset was inverted
using the same configuration as in the synthetic case, enabling a consistent
comparison across sampling densities.

The results, shown in Figure~\ref{fig_real_blockmean}a–h, reveal a clear and
systematic pattern across all tests. When coarse block-mean sizes are used,
the decimation removes a significant portion of the short-wavelength content,
leading to a visible loss of resolution in the predicted magnetic anomaly.
Important features associated with shallow or rapidly varying sources become
progressively smoothed as the block size increases, reflecting the intrinsic
spatial averaging introduced by the block-mean operation.

On the other hand, when very fine block-mean sizes are adopted, the results
do not continue to improve indefinitely. The limiting factor becomes the
...
refinement does not translate into additional resolved detail. At this scale,
the inversion is effectively sampling the data at a resolution finer than
what the grid can express, and the predicted fields remain essentially
unchanged.

Taken together, these observations show that the spherical gradient-boosted
equivalent sources behave consistently across sampling densities:
(i) coarse block-means degrade the recovered short-wavelength field, and
(ii) fine block-means converge toward a stable solution whose practical
limit is the grid resolution itself.

A key implication arises from these results. We find that using a block-mean
resolution close to the final grid-cell size already recovers the same
effective spatial resolution as finer decimated data, or even as the full
original dataset. This raises a central question for large-scale spherical
equivalent sources inversions: is it truly necessary to work with sampling finer than the
target grid if this increases computational cost without improving the
recovered field? Our experiments strongly suggest that block-mean
decimation with cell sizes comparable to, or only slightly finer than, the
prediction grid provides a more efficient and equally robust alternative for
regional to continental applications.

\begin{figure}[htb]
\centering
\includegraphics[width=1\linewidth]{figures/real_data_blockmean.png}
...
decomposition to a spherical framework, the proposed methodology
...
source framework provides an efficient and geometrically consistent
