...
\citep{Mendonca1994, Mendonca1995, Dampney1969, Li2006, Cordell1992, Ebbing2021}, and
...
Most equivalent source approaches are formulated in Cartesian coordinates, which
are well-suited for relatively small areas. To apply these methods to larger
regions, data natively registered in geodetic 
coordinates must first be projected onto a planar surface. Although this
projection intrinsically introduces distortions in distances, areas, and
...
conditions in longitude are required. Therefore, a spherical formulation of
the forward and inverse problem offers a mathematically rigorous alternative.
...
% Cartesian system, the mathematical and computational implementation of the
% forward problem is more time consuming than a spherical formulation because of
% the many coordinate conversions and rotations involved \citep{WildPfeiffer2008}.

The use of equivalent sources for modeling the Earth's magnetic field evolved through
...
the high-frequency residuals. Expanding on this sequential strategy,
\citet{Heincke2026} successfully utilized three equivalent source layers that
...
\begin{figure}[!ht]
    ...
    true synthetic fields (``True''). The second row \textbf{(e-h)} shows the
    fields predicted (``Predicted'') by the dual-layer model. The third row
    \textbf{(i-l)} shows the final residuals (``Residuals''), corresponding to
...
detailed configuration of the sources is provided in the source code archive
\citep{archive}.
...
For the modeling process, the equivalent sources in both the deep and shallow
...
Figure~\ref{fig_crossvalidation}a. The minimum RMSE of $83.6$ was obtained for a relative
...
The cross-validation results are shown in
Figure~\ref{fig_crossvalidation}b. The minimum RMSE of $357.5$ was obtained for a relative
...
\begin{figure}[tb]
    ...
    \textbf{a--c)} Observed total-field anomaly gridded from different
    block-mean decimations. \textbf{e--f)} Predicted total-field anomaly
...
using GPS, ensuring high positional accuracy. According to the official
processing report, the data were fully levelled using Oasis Montaj following a
two-stage procedure to minimize differences at flight line intersections. The
dataset was originally referenced to the SAD-69 datum and subsequently
converted to WGS-84 by the data provider.
...
the source code archive \citep{archive}.
...
While the grids produced from the complete dataset and the data decimated at
...
enhanced computational efficiency. Based on this optimal trade-off between
minimizing processing time and maximizing spectral preservation, the dataset
decimated at $0.0023^{\circ}$ was selected for the subsequent analyses.
The hyperparameters of the deep layer were selected through a block K-fold
...
The cross-validation results are shown in
Figure~\ref{fig_real_crossvalidation}a. The minimum RMSE of $77.3$ was obtained for a
...
The cross-validation results are shown in
Figure~\ref{fig_real_crossvalidation}b. The minimum RMSE of $356.6$ was obtained for the
...
A comprehensive summary of the hyperparameters and processing variables
used throughout our methodology, for both synthetic and real data
applications, is presented in Table~\ref{tab_parameters}.

\begin{table}[ht!]
    \centering
    \caption{Summary of the hyperparameters and processing parameters used for
    the dual-layer equivalent sources inversion in both synthetic and real
    data applications.}
    \label{tab_parameters}
    \begin{tabular}{llcc}
        \toprule
        \textbf{Layer} & \textbf{Parameter} & \textbf{Synthetic Data} & \textbf{Real Data} \\
        \midrule
        \textbf{Deep}  & Data decimation block size & $0.3^{\circ}$ ($\sim$33~km) & $0.1^{\circ}$ ($\sim$11~km) \\
                       & Window size                & N/A (entire dataset)      & N/A (entire dataset) \\
                       & CV spatial block size      & $1^{\circ}$ ($\sim$111~km)& $0.45^{\circ}$ ($\sim$50~km) \\
                       & Tested depth range         & 90--270~km                & 70--130~km \\
                       & Optimal relative depth     & 135~km                    & 100~km \\
                       & Tested damping range       & $10^{2}$--$10^{6}$        & $10^{0}$--$10^{4}$ \\
                       & Optimal damping            & $10^{3}$                  & $10^{2}$ \\
        \midrule
        \textbf{Shallow}& Gradient-boosting window  & $2^{\circ}$ ($\sim$220~km)& $1.5^{\circ}$ ($\sim$166~km) \\
                       & CV spatial block size      & $0.1^{\circ}$ ($\sim$12~km)& $0.18^{\circ}$ ($\sim$20~km) \\
                       & Tested depth range         & 15--30~km                 & 6--12~km \\
                       & Optimal relative depth     & 27~km                     & 9~km \\
                       & Tested damping range       & $10^{1}$--$10^{5}$        & $10^{2}$--$10^{6}$ \\
                       & Optimal damping            & $10^{2}$                  & $10^{4}$ \\
        \bottomrule
    \end{tabular}
\end{table}
...
sources modeling \citep{Uppal2025}, gradient-boosting \citep{Soler2021},
...
Overall, the results indicate that the spherical dual-layer gradient-boosted
...
model for continental scale data compilations. Additionally, future work
...
curvature on large-scale magnetic modeling.
...
We also thank editor Mark Dekkers, reviewer Maximilian Lowe and an annonymous reviewer 
for their feedback on the original manuscript.
