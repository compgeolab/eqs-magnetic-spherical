...
%DIF DEL /tmp/x79h80Qyh6/latexdiff-vc-submitted/manuscript.tex   Tue Mar 10 14:55:54 2026
%DIF ADD manuscript.tex                                          Wed Apr 29 10:47:20 2026
%
...
%DIF -------
\newcommand{\PreprintDOI}{10.31223/X5M19R} %DIF > 
%DIF -------
...
technique, but traditional \DIFaddbegin \DIFadd{modeling and gridding }\DIFaddend methods process this data as if the Earth were flat. 
...
\begin{enumerate}
  \item \DIFdelbegin \DIFdel{Equivalent sources methods in Cartesian coordinates using map projections
    introduce distortions at continental or global scales}\DIFdelend \DIFaddbegin \DIFadd{Spherical equivalent sources overcome planar projection limits,
  enabling global-scale magnetic modeling}\DIFaddend .
...
\citep{Mendonca1994, Mendonca1995, Dampney1969, Li2006, Cordell1992}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{Mendonca1994, Mendonca1995, Dampney1969, Li2006, Cordell1992, Ebbing2021}}\hskip0pt%DIFAUXCMD
}\DIFaddend , and
...
re\-para\-me\-trization of the equivalent layer \citep{OliveiraJr2013,Mendonca2020},
...
Most equivalent source approaches are formulated in Cartesian coordinates\DIFdelbegin \DIFdel{and applied to }\DIFdelend \DIFaddbegin \DIFadd{, which
are well-suited for }\DIFaddend relatively small areas. \DIFdelbegin \DIFdel{In these cases, data which are naturally registered using geodetic 
or geocentric
spherical coordinates are }\DIFdelend \DIFaddbegin \DIFadd{To apply these methods to larger
regions, data natively registered in geodetic 
coordinates must first be }\DIFaddend projected onto a planar surface\DIFdelbegin \DIFdel{using a chosen map
projection to use the Cartesian formulation. An issue arises when dealing with large areas where }\DIFdelend \DIFaddbegin \DIFadd{. Although this
projection intrinsically introduces }\DIFaddend distortions in distances, areas, \DIFdelbegin \DIFdel{or relative positioningfrom the projection become significant. Thus, the
limitation does not arise from the Cartesian formulation
itself, but from representing data distributed over the
curved
Earth on a planar surface. In this context}\DIFdelend \DIFaddbegin \DIFadd{and
relative positioning, quantifying the exact impact of these distortions
relative to typical survey uncertainties is an extensive task. However, the
fundamental limitation of Cartesian formulations becomes absolute at the
global scale, where no single map projection is adequate and periodic boundary
conditions in longitude are required. Therefore}\DIFaddend , a spherical formulation of
the forward and inverse problem \DIFdelbegin \DIFdel{is more suitable for
regional to continental-scale modeling because it avoids the projection and its
associated distortions.
While a Cartesian formulation could still be applied using a geocentric
Cartesian system, the
mathematical and computational implementation of the
forward problem is more time consuming than a spherical formulation because of
the many coordinate conversions and rotations involved \mbox{%DIFAUXCMD
\citep{WildPfeiffer2008}}\hskip0pt%DIFAUXCMD
.
}\DIFdelend \DIFaddbegin \DIFadd{offers a mathematically rigorous alternative.
By operating directly on the curved Earth and avoiding planar projections, the
spherical approach provides an exact solution that is uniquely capable of
seamless modeling from regional scales to global satellite datasets.
%DIF >  While a Cartesian formulation could still be applied using a geocentric
%DIF >  Cartesian system, the mathematical and computational implementation of the
%DIF >  forward problem is more time consuming than a spherical formulation because of
%DIF >  the many coordinate conversions and rotations involved \citep{WildPfeiffer2008}.
}\DIFaddend 

%DIF < The equivalent source method in spherical coordinates was first introduced by
%DIF < \citet{VonFrese1981} as a computationally efficient alternative to spherical harmonics,
%DIF < which become expensive at high degrees, and to Fourier transforms, which require planar
%DIF < regridding of spherical data. Their approach relates potential field anomalies directly to
%DIF < a distribution of equivalent point sources on a sphere through least squares inversion,
%DIF < allowing the anomalies to be reproduced with far fewer parameters than spherical harmonics.
%DIF < This formulation is numerically efficient because, once the source coefficients are solved,
%DIF < continuation, differentiation, and filtering operations can be carried out by simply
%DIF < recomputing the anomaly of the same source distribution at new conditions. Moreover, they
%DIF < demonstrated that magnetic anomalies could be differentially reduced to the pole using
%DIF < equivalent dipoles, thus enabling pole reduction directly in the spherical domain without
%DIF < distortion.
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend The use of equivalent sources for modeling the Earth's magnetic field evolved through
various formulations from the late 1960s to the early 1980s
\citep{Alldredge1969, Mayhew1979, Mayhew1980, Reilly1978, Mayhew1982}.
For instance, \citet{Mayhew1979} successfully applied an equivalent source
representation using arrays of dipoles and spherical prisms to model
satellite magnetic anomaly data. Later,
\citet{VonFrese1981} developed a systematic procedure for the comprehensive
geologic analysis of both gravity and magnetic anomalies in spherical
coordinates. They formalized the equivalent point source inversion as a
computationally efficient alternative to spherical harmonics, which become
computationally challenging at high degrees, and to Fourier transforms, which require planar
projection of spherical data. Their approach relates potential field anomalies
directly to a distribution of equivalent point sources on a sphere through
least squares matrix inversion.
...
using CHAMP satellite data. They placed approximately $30,000$ monopoles on
...
the high-frequency residuals. \DIFaddbegin \DIFadd{Expanding on this sequential strategy,
\mbox{%DIFAUXCMD
\citet{Heincke2026} }\hskip0pt%DIFAUXCMD
successfully utilized three equivalent source layers that
are subsequently fitted to model large-scale magnetic anomalies.
}\DIFaddend 
...
within which a geocentric spherical system ($r, \theta, \lambda$) is used to formulate the physics
...
$H_{\lambda r}$ and $H_{\lambda \theta}$ in Equations~\ref{eq:H_lambdar} and \ref{eq:H_lambdatheta}
incorporate the correction proposed by \citet{Dyment1998}.
During our implementation, we verified that
this correction is consistent with the expressions adopted in
subsequent studies. In particular, a discrepancy in the direction
cosines associated with these terms was identified and corrected
accordingly.
...
the angular difference between the geodetic ($\varphi$) and geocentric ($\phi$) latitudes,
$\Delta\Phi = \varphi - \phi$ (Figure~\ref{fig_coordinate_systems}b). As part of our workflow, the
input magnetic moment vector of a source, $\vec{\mathbf{m}}$, with geodetic components $(m_{x^{\dagger}}, m_{y^{\dagger}}, m_{z^{\dagger}})$,
is first rotated into the local spherical frame to obtain the spherical components $(m_x, m_y, m_z)$
...
spherical components $(B_x, B_y, B_z)$, is rotated back into the local geodetic frame to
obtain the output components $(B_{x^{\dagger}}, B_{y^{\dagger}}, B_{z^{\dagger}})$. This inverse transformation is achieved using the transpose
...
in studies like \citet[][p. 179]{Blakely1995}, the total-field anomaly, $\Delta T$, can be approximated as the
...
\begin{equation}
    d_i = \sum_{j=1}^{M} A_{ij} p_j,
\end{equation}
\noindent
where $p_j = \|\vec{\mathbf{m}}_j\|$ is the magnetic moment magnitude of the $j$-th source, and $A_{ij}$ is the element of
...
unit magnetic moment at the $i$-th observation point, as derived in the previous section in
equation~\ref{eq:magnetic_field_sph}.
...
The inverse problem consists of estimating the parameter vector $\mathbf{p}$,
containing the $M$ magnetic moment amplitudes, that best
fits the observed data $\mathbf{d}^o$. Since this problem is typically ill-posed, we introduce a
Tikhonov regularization term, leading to a damped least-squares formulation \citep{Soler2021}.
The objective function is formulated using a scaling factor as

\begin{equation}
    \beta(\mathbf{c}) = [\mathbf{d}^o - \mathbf{U}\mathbf{c}]^\mathsf{T}
    \mathbf{W} [\mathbf{d}^o - \mathbf{U}\mathbf{c}] + \lambda \, \mathbf{c}^\mathsf{T}\mathbf{c},
    \label{eq:objective-function}
\end{equation}

\noindent
where $\mathbf{W}$ is a diagonal matrix of data weights, $\mathbf{U} = \mathbf{A}\mathbf{S}^{-1}$ is
the scaled sensitivity matrix, $\mathbf{c} = \mathbf{S}\mathbf{p}$ is the scaled
parameter vector, and $\lambda$ is a dimensionless damping parameter. The scaling
...
makes the vector $\mathbf{c}$ have data units and narrows the effective search range for $\lambda$, which can
...
\begin{equation}
    (\mathbf{U}^\mathsf{T}\mathbf{W}\mathbf{U} + \lambda \mathbf{I}) \, \hat{\mathbf{c}}
    = \mathbf{U}^\mathsf{T}\mathbf{W}\mathbf{d}^o,
    \label{eq:normal-equations}
\end{equation}

\noindent
whose solution gives the scaled parameters $\hat{\mathbf{c}}$. The estimated magnetic moments are
then obtained by removing the scaling

\begin{equation}
    \hat{\mathbf{p}} = \mathbf{S}^{-1}\hat{\mathbf{c}}.
\end{equation}

Once the vector of moments $\hat{\mathbf{p}}$ is estimated, it can be used not only to reproduce the
observed total-field anomaly but also to compute the full anomalous magnetic field vector $\vec{\mathbf{B}}$
and its components at any location

\begin{equation}
    \vec{\mathbf{B}}({r, \theta, \lambda}) = \sum_{j=1}^{M}
    \vec{\mathbf{b}_{j}}({r, \theta, \lambda}) \, \hat{p}_j.
\end{equation}
...
To accurately model complex magnetic anomalies, we employ the dual-layer
gradient-boosted equivalent sources method introduced by \citet{Uppal2025}, illustrated in Figure~\ref{fig:dual_layer_flowchart},
this methodology first fits a deep layer of equivalent sources using block-averaged
or decimated data to capture the regional, long-wavelength signals.
Subsequently, a shallow layer is fitted to the residuals of the deep layer to
resolve the short-wavelength components. The final predicted field
is then obtained by combining the contributions from both layers.

\begin{figure}[htpb]
\centering
\includegraphics[width=\textwidth]{figures/fluxogram-dual-layer.png}
\caption{Flowchart of the dual-layer equivalent sources method of
  \citet{Uppal2025}.
  First, the original data (red hexagons) are decimated using a blocked average
  procedure and then deep sources (green circles) are fitted to the decimated
  data. The residuals (purple hexagons) between the original data and the deep
  source predictions are calculated and shallow sources (blue circles) are
  fitted to them. Finally, the combined effect is predicted on a regular grid (black
  triangles) with constant height using both sets of sources.
}
\label{fig:dual_layer_flowchart}
\end{figure}
...
\begin{equation}
    \mathbf{d} = \sum_{k=1}^{K} \mathbf{A}_k \mathbf{p}_k,
\end{equation}

\noindent
where $\mathbf{A}_k$ is the sensitivity matrix relating the sources in the $k$-th window to all
observation points, and $\mathbf{p}_k$ is the vector of corrective source moments estimated in that
window.
...
\begin{algorithmic}[1]
\State Initialize the global moment vector $\mathbf{p} = \mathbf{0}$.
...
\For{each window index $k$ in the shuffled list}
    ...
    \State Solve for the corrective moments $\mathbf{p}_k$:
    \Statex \hspace{\algorithmicindent} $\mathbf{p}_k = (\tilde{\mathbf{A}}_k^\mathsf{T} \tilde{\mathbf{A}}_k + \mu_k \mathbf{I})^{-1} \tilde{\mathbf{A}}_k^\mathsf{T} \tilde{\mathbf{r}}_{k-1}$
    \State Calculate the global prediction from the local sources: $\mathbf{d}_k
    = \mathbf{A}_k \mathbf{p}_k$.
    \State Update the global residuals: $\mathbf{r}_k = \mathbf{r}_{k-1} - \mathbf{d}_k$.
    \State Accumulate the global solution by adding $\mathbf{p}_k$ to the
    corresponding entries in $\mathbf{p}$.
\EndFor
\end{algorithmic}
\end{algorithm}
...
to generalize. The resulting error metric of how the model would perform
when predicting for a new, unsurveyed area, which is more realistic to use-case scenario.
...
\begin{algorithm}
    ...
    \For{each hyperparameter set $\theta$ in $\Theta$}
        ...
        \For{$i = 1$ to $k$}
            ...
            \State Fit model $\mathbf{p}_i$ on the training set using hyperparameters $\theta$.
...
\begin{figure}[!ht]
    ...
    \DIFaddbeginFL \DIFaddFL{true synthetic }\DIFaddendFL fields \DIFdelbeginFL \DIFdelFL{predicted }\DIFdelendFL (``\DIFdelbeginFL \DIFdelFL{Predicted}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{True}\DIFaddendFL '')\DIFdelbeginFL \DIFdelFL{by the dual-layer model}\DIFdelendFL . The second row \textbf{(e-h)} shows the
    \DIFdelbeginFL \DIFdelFL{true synthetic }\DIFdelendFL fields \DIFaddbeginFL \DIFaddFL{predicted }\DIFaddendFL (``\DIFdelbeginFL \DIFdelFL{True}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Predicted}\DIFaddendFL '') \DIFdelbeginFL \DIFdelFL{for comparison}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{by the dual-layer model}\DIFaddendFL . The third row
    \textbf{(i-l)} shows the final residuals (``Residuals\DIFdelbeginFL \DIFdelFL{"}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{''}\DIFaddendFL ), corresponding to
    the difference between the true and predicted fields.}
    \label{fig_grid_results}
\end{figure}

To evaluate the performance and accuracy of the proposed methodology, we applied
it to a synthetic magnetic dataset designed to reproduce realistic survey
conditions. The model consists of multiple crustal sources. Specifically, a set
of dipoles and dikes with different orientations (W-E, SW-NE, and NW-SE)
positioned at varying depths. These sources exhibit both remanent and induced
magnetization, generating both regional and local magnetic anomalies. The
detailed configuration of the sources is provided in the supplementary material
\citep{archive}.
...
\DIFaddbegin \DIFadd{For the modeling process, the equivalent sources in both the deep and shallow
layers were defined as magnetic dipoles with an assigned magnetization direction
of $90^{\circ}$ inclination and $0^{\circ}$ declination.
}\DIFaddend The deep layer was estimated from a block-reduced version of the synthetic
...
resulting in 25 parameter combinations. Spatial blocks of 1$^{\circ}$ 
(approximately 111~km) were used to avoid the spatial dependence between samples
in the training and testing sets during the cross-validation process.
The cross-validation results are shown in
Figure~\ref{fig_crossvalidation}a. The minimum RMSE \DIFaddbegin \DIFadd{of $83.6$ }\DIFaddend was obtained for a relative
...
computationally heavy due to the large number of observations.
However, since
...
The cross-validation results are shown in
Figure~\ref{fig_crossvalidation}b. The minimum RMSE \DIFaddbegin \DIFadd{of $357.5$ }\DIFaddend was obtained for a relative
...
\begin{figure}[tb]
    ...
    \DIFdelbeginFL \textbf{\DIFdelFL{a--d)}} %DIFAUXCMD
\DIFdelendFL \DIFaddbeginFL \textbf{\DIFaddFL{a--c)}} \DIFaddendFL Observed total-field anomaly gridded from different
    block-mean decimations. \DIFdelbeginFL \textbf{\DIFdelFL{e--h)}} %DIFAUXCMD
\DIFdelendFL \DIFaddbeginFL \textbf{\DIFaddFL{e--f)}} \DIFaddendFL Predicted total-field anomaly
    using the spherical dual-layer gradient-boosted equivalent sources.}
...
\url{https://reate.cprm.gov.br/anp/TERRESTRE}), covers latitudes from
$-24.14^{\circ}$ to $-14.14^{\circ}$ and longitudes from $-56.44^{\circ}$ to
$-46.96^{\circ}$, encompassing one of the largest sedimentary basins in South
America. Data acquisition was conducted along north--south flight lines spaced
...
using GPS, ensuring high positional accuracy. \DIFaddbegin \DIFadd{According to the official
processing report, the data were fully levelled using Oasis Montaj following a
two-stage procedure to minimize differences at flight line intersections. }\DIFaddend The
dataset was originally referenced to the SAD-69 datum and subsequently
converted to WGS-84 \DIFdelbegin \DIFdel{using
official transformation parameters}\DIFdelend \DIFaddbegin \DIFadd{by the data provider}\DIFaddend .
...
addition, tie lines were removed because they exhibited inconsistent amplitude values,
being entirely positive and showing amplitudes ten times greater than the
flight line data, introducing spurious variations and systematic
...
1,636,695 observations is illustrated in Figure~\ref{fig_real_blockmean}a.
...
\DIFaddbegin \DIFadd{While the grids produced from the complete dataset and the data decimated at
$0.0023^{\circ}$ appear visually identical at a regional scale, closer inspection
of the short-wavelength features reveals the true impact of the decimation
strategy. The coarser block-mean decimation ($0.06^{\circ}$) significantly
smooths out the sharp, high-frequency anomalies associated with near-surface
sources. In contrast, decimation using a block size compatible with the final
grid resolution ($0.0023^{\circ}$) exceptionally preserves these high-frequency
details, yielding results strictly equivalent to the full dataset but with
enhanced computational efficiency. }\DIFaddend Based on this \DIFaddbegin \DIFadd{optimal }\DIFaddend trade-off between
\DIFdelbegin \DIFdel{computational efficiency and }\DIFdelend \DIFaddbegin \DIFadd{minimizing processing time and maximizing }\DIFaddend spectral preservation, the dataset
decimated \DIFdelbegin \DIFdel{with a block size of }\DIFdelend \DIFaddbegin \DIFadd{at }\DIFaddend $0.0023^{\circ}$ was selected for the subsequent analyses.
The hyperparameters of the deep layer were selected through a block K-fold
...
The cross-validation results are shown in
Figure~\ref{fig_real_crossvalidation}a. The minimum RMSE \DIFaddbegin \DIFadd{of $77.3$ }\DIFaddend was obtained for a
...
The cross-validation results are shown in
Figure~\ref{fig_real_crossvalidation}b. The minimum RMSE \DIFaddbegin \DIFadd{of $356.6$ }\DIFaddend was obtained for the
...
%DIFDELCMD < %%%
\DIFdel{This study presented and systematically validated a spherical equivalent
sources framework for large-scale magnetic data processing , interpolation, and
transformation. The proposed methodologyintegrates dual-layer equivalent
sources modeling \mbox{%DIFAUXCMD
\citep{Uppal2025}}\hskip0pt%DIFAUXCMD
, gradient-boosting
\mbox{%DIFAUXCMD
\citep{Soler2021}}\hskip0pt%DIFAUXCMD
, and spatial cross-validation into a unified and
computationally efficient workflow.
Our equivalent sources formulation can work with data
in both spherical and
geodetic coordinate systems}\DIFdelend \DIFaddbegin \DIFadd{A comprehensive summary of the hyperparameters and processing variables
used throughout our methodology, for both synthetic and real data
applications, is presented in Table~\ref{tab_parameters}}\DIFaddend .

\DIFaddbegin \begin{table}[ht!]
    \centering
    \caption{\DIFaddFL{Summary of the hyperparameters and processing parameters used for
    the dual-layer equivalent sources inversion in both synthetic and real
    data applications.}}
    \label{tab_parameters}
    \begin{tabular}{llcc}
        \toprule
        \textbf{\DIFaddFL{Layer}} & \textbf{\DIFaddFL{Parameter}} & \textbf{\DIFaddFL{Synthetic Data}} & \textbf{\DIFaddFL{Real Data}} \\
        \midrule
        \textbf{\DIFaddFL{Deep}}  & \DIFaddFL{Data decimation block size }& \DIFaddFL{$0.3^{\circ}$ ($\sim$33~km) }& \DIFaddFL{$0.1^{\circ}$ ($\sim$11~km) }\\
                       & \DIFaddFL{Window size                }& \DIFaddFL{N/A (entire dataset)      }& \DIFaddFL{N/A (entire dataset) }\\
                       & \DIFaddFL{CV spatial block size      }& \DIFaddFL{$1^{\circ}$ ($\sim$111~km)}& \DIFaddFL{$0.45^{\circ}$ ($\sim$50~km) }\\
                       & \DIFaddFL{Tested depth range         }& \DIFaddFL{90--270~km                }& \DIFaddFL{70--130~km }\\
                       & \DIFaddFL{Optimal relative depth     }& \DIFaddFL{135~km                    }& \DIFaddFL{100~km }\\
                       & \DIFaddFL{Tested damping range       }& \DIFaddFL{$10^{2}$--$10^{6}$        }& \DIFaddFL{$10^{0}$--$10^{4}$ }\\
                       & \DIFaddFL{Optimal damping            }& \DIFaddFL{$10^{3}$                  }& \DIFaddFL{$10^{2}$ }\\
        \midrule
        \textbf{\DIFaddFL{Shallow}}& \DIFaddFL{Gradient-boosting window  }& \DIFaddFL{$2^{\circ}$ ($\sim$220~km)}& \DIFaddFL{$1.5^{\circ}$ ($\sim$166~km) }\\
                       & \DIFaddFL{CV spatial block size      }& \DIFaddFL{$0.1^{\circ}$ ($\sim$12~km)}& \DIFaddFL{$0.18^{\circ}$ ($\sim$20~km) }\\
                       & \DIFaddFL{Tested depth range         }& \DIFaddFL{15--30~km                 }& \DIFaddFL{6--12~km }\\
                       & \DIFaddFL{Optimal relative depth     }& \DIFaddFL{27~km                     }& \DIFaddFL{9~km }\\
                       & \DIFaddFL{Tested damping range       }& \DIFaddFL{$10^{1}$--$10^{5}$        }& \DIFaddFL{$10^{2}$--$10^{6}$ }\\
                       & \DIFaddFL{Optimal damping            }& \DIFaddFL{$10^{2}$                  }& \DIFaddFL{$10^{4}$ }\\
        \bottomrule
    \end{tabular}
\end{table}
...
\DIFaddend The synthetic data experiments demonstrated that the dual-layer configuration
...
conditions. The adopted data decimation strategy for the deep layer reduced
...
The cross-validation of the shallow layer exhibited a less well-defined
minimum, reflecting the decreased sensitivity of near-surface sources to
depth and regularization. These results highlight the importance of
independent parameter tuning and justify the adoption of separate
optimization strategies for regional and local components.
Moreover, our synthetic data results show that our spherical dual-layer
method is able to accurately recover the magnetic field components from the
total-field anomaly observations. This was shown to be theoretically possible
through the use of Fourier transform by \citet{Lima2009} and through the use
of equivalent sources by \citet{Li2020}. Here, we demonstrate that this can
also be achieved in a spherical or geodetic reference frame through the use
...
and large data volumes (upward of 1.5 million data points). A detailed
analysis of different spatial decimation strategies demonstrated that
decimation using coarse block sizes (of the order of the line spacing),
although computationally efficient, leads to significant attenuation of
short-wavelength anomalies and compromises the recovery of near-surface
sources. Conversely, data decimation using a block size that is compatible
with the final grid resolution preserved the amplitude of short-wavelength
anomalies and resulted in a grid that is largely indistinguishable from the
one produced with the full dataset. Moreover, the processing time for the
reduced dataset was approximately 20\% smaller than for the full dataset.
Therefore, we recommend always decimating the input data using a block mean
operation with block size equal to the desired grid spacing \citep{Smith1990}.
...
avoiding noise amplification. The anomalous field amplitude is particularly
useful for interpretation and geometry inversion workflows.
As demonstrated by \citet{HidalgoGato2021}, the inversion of basement relief
from the amplitude of the magnetic field is only weakly dependent on the
often unknown magnetization direction. This characteristic allows reliable
structural characterization even in the presence of remanent magnetization
and limited prior information. Consequently, the framework proposed here
offers a solid foundation for subsequent structural mapping, susceptibility
inversion, and integrated geophysical modeling.
...
\DIFadd{This study presented and systematically validated a spherical equivalent
sources framework for large-scale magnetic data continuation, interpolation,
and transformation. The proposed methodology integrates dual-layer equivalent
sources modeling \mbox{%DIFAUXCMD
\citep{Uppal2025}}\hskip0pt%DIFAUXCMD
, gradient-boosting \mbox{%DIFAUXCMD
\citep{Soler2021}}\hskip0pt%DIFAUXCMD
,
and spatial cross-validation into a unified and computationally efficient
workflow. Our equivalent sources formulation can work with data in both
spherical and geodetic coordinate systems.
}

\DIFaddend Overall, the results indicate that the spherical dual-layer gradient-boosted
equivalent sources method constitutes a flexible, scalable, and physically
consistent solution for magnetic data processing. The framework is capable of
handling very large datasets while preserving short- and long-wavelength of
the magnetic field and maintaining manageable computational requirements.
The method proposed here is able to handle millions of data points in a modest
computer in the timescale of hours. However, the spherical calculations are
significantly slower than their Cartesian counterparts. Thus, the current
formulation will likely struggle to process larger compilations involving
multiple surveys and tens to hundreds of millions of data points without
...
model for continental scale data compilations\DIFaddbegin \DIFadd{. Additionally, future work
will focus on a comprehensive quantitative comparison between our spherical
framework and traditional Cartesian approaches. This comparison will allow
us to rigorously assess the impact of map projection distortions and Earth's
curvature on large-scale magnetic modeling}\DIFaddend .
...
Verde \citep{Uieda2018} for spatial interpolation and data decimation,
Bordado \citep{bordado} for moving window operations,
Harmonica \citep{Harmonica2024} for potential field data
processing and modelling,
Boule \citep{boule} for ellipsoids and coordinate conversions,
and
Jupyter Notebooks \citep{Granger2021,KluyverThomas2016} for code development and documentation.
...
LU was supported in part by start-up grant PRPI 22.1.09345.01.2 from
Universidade de São Paulo.
We are grateful to Santiago R. Soler, Gelson F. Souza-Junior, Yago M. Castro,
Vanderlei C. Oliveira Jr., and Marco Couto Junior for discussions that helped
shape and refine this research.
